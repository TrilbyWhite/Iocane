...
\item[\Opt{button} \Arg{n}]
	Simulate a press (and release) of mouse button \Arg{n}
\item[\Opt{hold} \Arg{n}]
	Simulate a press without release of mouse button \Arg{n}
\item[\Opt{release} \Arg{n}]
	Simulate a release of mouse button \Arg{n}
...
\item[\Opt{toggle}]
    ...
    This is used to keep \Prog{iocane} running in interactive mode but use the
    bound keys temporarily for some other program.
    The key bound to this command is still grabbed by \Prog{iocane}, though.
\end{description}
